...
contract PreToken is ReentrancyGuard {
    ...
    function redeem(address account) external nonReentrant {
        ...
        uint256 keyAmount = totalTokenBalance * contribution / totalBaseContributed;
        IERC20(token).transfer(account, keyAmount);
        emit PreToken__Redeemed(account, keyAmount);
    }
...
contract Token is ERC20, ERC20Permit, ERC20Votes, ReentrancyGuard {
        ...
        override(ERC20)
    {
        ...
        if (amount > getAccountTransferrable(from)) revert Token__CollateralRequirement();
        _updateFor(from);
        _updateFor(to);
    }
