...
contract Token is ERC20, ERC20Permit, ERC20Votes, ReentrancyGuard {
        ...
        ERC20Permit(_name) 
    {
        ...
        if (quoteDecimals > 18) revert Token__QuoteDecimals();
        ...
        returns (uint256 tokenAmtOut)
    {
        ...
        uint256 y1 = x0.mulWadUp(reserveTokenAmt).divWadUp(x1);
        tokenAmtOut = reserveTokenAmt - y1;
        ...
        returns (uint256 quoteRawOut)
    {
        ...
        uint256 x0 = reserveVirtQuoteWad + reserveRealQuoteWad;
        uint256 y1 = reserveTokenAmt + netAmt;
        if (y1 == 0) revert Token__DivideByZero(); 

        uint256 x1 = x0.mulWadUp(reserveTokenAmt).divWadUp(y1);
        ...
        returns (uint256 creditRaw)
    {
        ...
        uint256 required = xv.mulWadDown(m).divWadDown(m - balance);
        uint256 creditLimitWad = required - xv;
        ...
        returns (uint256 tokenAmt) 
    {
        ...
        uint256 required = xv + rawToWad(debtRaw);
        uint256 nonLocked = xv.mulWadDown(m).divWadDown(required);
